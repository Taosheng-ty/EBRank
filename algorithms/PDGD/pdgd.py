...
class PDGD(BasicOnlineRanker):
  ...
  def __init__(self, learning_rate, learning_rate_decay,
    ...
    self.parent_parameters=PDGD.default_parameters()
  ...
  #   scores = -self.LearningModel.score(features)
  ...
  def _create_train_ranking(self, query_id, query_feat, inverted=False,data_split=None,**kwargs):
    ...
    label_vector=data_split.query_values_from_vector(query_id,data_split.label_vector)
    exp=data_split.query_values_from_vector(query_id,data_split.exposure)
    # print(docFreq)
    n_docs = query_feat.shape[0]

    k = np.minimum(self.n_results, n_docs)
    self.doc_scores = self.LoggingModel.score(query_feat)
    self.doc_scores = self.doc_scores+18 - np.amax(self.doc_scores)
  ...
  def _update_to_clicks(self, clicks):
    ...
    self.LearningModel._last_features=self.LoggingModel._last_features
    self.LearningModel.update_to_documents(all_ind,
                                   all_w)
...
class PDGDv1(PDGD):
  def __init__(self, *args, **kargs):
    super(PDGDv1, self).__init__(*args, **kargs)
  def _update_to_clicks(self, clicks):
    n_docs = self.ranking.shape[0]
    cur_k = np.minimum(n_docs, self.n_results)
    included = np.ones(cur_k, dtype=np.int32)
